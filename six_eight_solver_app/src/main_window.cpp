...
namespace dak::six_eight_solver_app
{
   ...
   static QPainterPath convert_tile_line(const tile_t& a_tile, const six_eight::position_t& a_pos, const QPointF& a_tile_center, double a_tile_radius, const six_eight::color_t& a_color, const QColor& a_qt_color)
   {
   ...
   static std::map<six_eight::color_t, QColor> color_conversions =
   {
      { six_eight::color_t::red(),    QColor(220, 0, 0) },
      { six_eight::color_t::green(),  QColor(0, 220, 0) },
      { six_eight::color_t::blue(),   QColor(10, 10, 240) },
      { six_eight::color_t::yellow(), QColor(220, 220, 0) },
   };

   const double tile_radius = 50;

   static void draw_tile_in_scene(const tile_t& a_tile, const six_eight::position_t& a_pos, QGraphicsScene& a_scene)
   {
   ...
   static void draw_tile_selection(bool is_selected, const six_eight::position_t& a_pos, QGraphicsScene& a_scene)
   {
      const QPointF tile_center = convert_tile_pos(a_pos, tile_radius);
   ...
   void main_window_t::draw_selected_puzzle_tile()
   {
      ...
      for (const auto& tile : my_puzzle->initial_tiles())
      {
         ...
         const six_eight::position_t pos(tile_x * 2 - tile_y, tile_y * 2);
   ...
   void main_window_t::solve_puzzle()
   {
      ...
      my_async_solving = std::async(std::launch::async, [self = this, puzzle = my_puzzle]()
      {
         try
         {
            return six_eight::solve(*puzzle, *self);
         }
