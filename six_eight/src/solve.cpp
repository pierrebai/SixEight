...
namespace dak::six_eight
{
      ...
      solve_context_t& a_ctx)
   {
      ...
      {
         // std::cout << "No more sub-puzzles for\n" << new_partial << std::endl;
         if (a_ctx.puzzle.is_solution_valid(new_partial))
         {
   ...
   static void solve_sub_puzzle_with_tile(const sub_puzzle_t& a_sub_puzzle, const solution_t& partial_solution, solve_context_t& a_ctx)
   {
      ...
      for (const auto new_pos : next_positions)
      {
         for (int selected_orientation = 0; selected_orientation < 4; ++selected_orientation)
         {
            ...
            if (partial_solution.is_compatible(tile, adjusted_new_pos))
            {
               solve_recursion(a_sub_puzzle, partial_solution, tile, adjusted_new_pos, a_ctx);
            }
         }
      }
   }
   ...
   all_solutions_t solve(const puzzle_t& a_puzzle, progress_t& a_progress)
   {
      ...
      // #define _DEBUG_WAWA
         ...
         solve_context_t ctx(threaded_worker, a_puzzle, mt_progress, 0);
